...
class ContextFreeGrammar{
	...
	std::vector<Rule> rules; // rules, productions 
	Nonterminal S; // start symbol
	
	public: ContextFreeGrammar(char * fileCFG){
		std::ifstream ifs(fileCFG, std::ifstream::in);
		std::string newline;
		int i = 0;
		while(std::getline (ifs, newline)){
			// std::cout << " newline : " << newline << std::endl;
			// std::cout << new Rule(newline) << std::endl;
			rules.push_back(Rule(newline));
		}
...
int main(int argc, char *argv[]){
	for(int index = 1; index <= argc; index++){
		ContextFreeGrammar cfg = ContextFreeGrammar(argv[index]);
